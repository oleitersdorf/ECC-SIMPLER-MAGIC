...
class SIMPLER_Top_Data_Structure:
    ...
    def PrintCodeGeneration(self):    
        ...
        # Reliability adjustments implemented here
        # First runs the SIMPLER algorithm, then encounters the following greedy algorithm to schedule additional operations required
        # Not ideal to implement in this function, yet entails a simple method with minimal adjustments to the SIMPLER algorithm.

        # Dictionaries detailing the operations in each cycle (keys are cycle numbers)
        mem_operations = {}
        cmem_operations = {}
        ...
        for node in mergerd_list:
            ...
            if (node.GetNodeOp() == NodeData.Get_Initialization_op_val() or node.GetNodeTime() == 0):
                continue

            # Add cycles as long as the mem is busy at this time
            while(current_cycle in mem_operations):
                current_cycle += 1
            ...
            # Schedule the mem operation for this time            
            mem_operations[current_cycle] = node_name

            if (node.GetNumOfOutEdges() == 0): #output

                # current_cycle represent sthe operation is calcualted.
                # current_cycle - 6 will represent the time at which MEMB->PA is calculated.
                # current_cycle - 5 will represent the time at which CMEM->PA is calculated.

                if ((current_cycle - 6) in mem_operations and (current_cycle - 6) not in cmem_operations):

                    # advance one
                    mem_operations = {((cycle + 1) if cycle >= (current_cycle - 6) else cycle):value for cycle, value in mem_operations.items()}
                    cmem_operations = {((cycle + 1) if cycle >= (current_cycle - 6) else cycle):value for cycle, value in cmem_operations.items()}

                    mem_operations[current_cycle-6] = node_name + " - MEMB to PA (START)"
                    cmem_operations[current_cycle-6] = node_name + " - MEMB to PA (START)" # labels that the use of a processing area begins now
                    cmem_operations[current_cycle-5] = node_name + " - CMEM to PA"

                    # update current_cycle to point to my operation
                    current_cycle += 1

                elif ((current_cycle - 6) not in mem_operations and (current_cycle - 6) not in cmem_operations):

                    # if completely free
                    if ((current_cycle - 5) not in cmem_operations):
                        mem_operations[current_cycle-6] = node_name + " - MEMB to PA (START)"
                        cmem_operations[current_cycle-6] = node_name + " - MEMB to PA (START)"
                        cmem_operations[current_cycle-5] = node_name + " - CMEM to PA"

                    # if -5 not free for cmem
                    else:
                        ...
                        # update current_cycle to point to my op
                        current_cycle += 1

                else:

                    # advance two
                    mem_operations = {((cycle + 2) if cycle >= (current_cycle - 6) else cycle):value for cycle, value in mem_operations.items()}
                    cmem_operations = {((cycle + 2) if cycle >= (current_cycle - 6) else cycle):value for cycle, value in cmem_operations.items()}
                ...
                # schedule the MEMA->PA operation
                current_cycle += 1 
                while(current_cycle in mem_operations or current_cycle in cmem_operations):
                    current_cycle += 1
                mem_operations[current_cycle] = node_name + " - MEMA to PA"
                cmem_operations[current_cycle] = node_name + " - INIT(CMEM)"

                # schedule the PA->CMEM operation
                e11_cmem_cycle = current_cycle + 5
                while(e11_cmem_cycle in cmem_operations):
                    e11_cmem_cycle += 1
                cmem_operations[e11_cmem_cycle] = node_name + " - PA to CMEM (END)"

            current_cycle += 1

        self.Intrl_Print('}')         

        # Calculate maximum pipeline depth (number of PAs used)
        start = min(min(mem_operations.keys()), min(cmem_operations.keys()))
        end = max(max(mem_operations.keys()), max(cmem_operations.keys()))
        pipeline_depth = 0
        max_depth = 0
        # Calculate num cycles for no processing areas
        total_num_cycles_no_pa = end - start
        for cycle in range (start, end + 1):

            mem_cycle_operation = mem_operations[cycle] if (cycle in mem_operations) else ""
            cmem_cycle_operation = cmem_operations[cycle] if (cycle in cmem_operations) else ""
            if ("START" in cmem_cycle_operation):
                pipeline_depth += 1
            elif ("END" in cmem_cycle_operation):
                pipeline_depth -= 1
            if (max_depth < pipeline_depth):
                max_depth = pipeline_depth
            # Print out the operations performed at each cycle
        ...
        print ('Total number of baseline cycles:',self.t)
        print ('Total number of cycles - with check:', end - start + self.lr - self.lc)
        print ('Total number of cycles - with check and no PA:', total_num_cycles_no_pa + self.lr - self.lc)
